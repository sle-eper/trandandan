...
server.ready().then(() => {
  ...
  io.on("connection", async (socket) => {
    ...
    socket.on('accept_play', async (id, friendId) => {
      try {
        console.log(`[CHAT] accept_play received from ${id} for friend ${friendId}`);
        if (!id || !friendId) return;
        ...
        const mySockets = onlineUsers.get(myId);
        const friendSockets = onlineUsers.get(fId);

        console.log(`[CHAT] Sockets found - Me (${myId}): ${mySockets?.size || 0}, Friend (${fId}): ${friendSockets?.size || 0}`);

        // Notify both parties to start the game
        console.log(`[CHAT] Challenge accepted. GameId: ${gameId}`);

        if (mySockets) {
          for (const sid of mySockets) {
            console.log(`[CHAT] Emitting start_game to my socket: ${sid}`);
        ...
        if (friendSockets) {
          for (const sid of friendSockets) {
            console.log(`[CHAT] Emitting start_game to friend socket: ${sid}`);
